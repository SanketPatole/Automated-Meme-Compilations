...
videos_path = current_project_path + '/videos_the_world_of_dogs/'
html = urllib.request.urlopen("https://www.youtube.com/results?search_query=funny+dogs&sp=EgQIAxgD")
video_ids = re.findall(r"watch\?v=(\S{11})", html.read().decode())

INTRO_VID = 'the_world_of_dogs_intro.mp4'	# SET AS '' IF YOU DON'T HAVE ONE
OUTRO_VID = 'funniest_meme_compilations_outro.mp4'
TOTAL_VID_LENGTH = 60*60
MAX_CLIP_LENGTH = 10*60
MIN_CLIP_LENGTH = 0
now_timestamp = datetime.datetime.now()
outputFile = "the_world_of_dogs_" + num_to_month[now_timestamp.month].upper() + "_" + str(now_timestamp.year) + "_v" + str(
	now_timestamp.day) + ".mp4"

videonumber = date.today() - date(2021, 12, 31)
videonumber = videonumber.days
title = "MEME MARATHON | Funniest Dog Videos Compilation Volume #" + str(videonumber)
description = "MEME MARATHON | Funniest Dog Videos Compilation Volume #" + str(
	videonumber) + "\n\nEnjoy the memes! :)\n\nlike and subscribe to @The World Of Dogs for more Videos\n\nCopyright Disclaimer, Under Section 107 of the Copyright Act 1976, allowance is made for 'fair use' for purposes such as criticism, comment, news reporting, teaching, scholarship, and research. Fair use is a use permitted by copyright statute that might otherwise be infringing. Non-profit, edudogional or personal use tips the balance in favor of fair use.\n\nI as an individual creator do not earn any money from these compilation videos due to 6-7 copyright claims on all of the videos, and YouTube also does not monetize these compilation channels.\nIf you want me to keep making these amazing compilation videos, please become a Patron, support me and do not forget to subscribe to my channel.\nhttps://www.patreon.com/join/funniestmemecompilations\n\n\n\n#funniestdogvideos #compilation #funny #funnyvideos\n\n\n\nCredits (Please hit a like to the original videos and subscribe to their channels)\n\n"
tags = ['baby dogs', 'baby dog', 'dog baby', 'cute baby', 'cute dogs', 'cute dog', 'funny dogs', 'funny dog', 'cutest dogs', 'cutest dog', 'cute baby dogs', 'cute baby dog', 'funny baby dogs', 'funny dog videos', 'cute dog videos', 'dog videos', 'aww animals', 'dog compilation', 'dog videos compilation', 'baby animals', 'cute videos', 'funny videos']
...
for video_id in video_ids:
	try:
		...
		if youtube.title.lower().find("react") == -1 and youtube.title.lower().find("reacts") == -1 and youtube.title.lower().find("reaction") == -1 and youtube.description.lower().find("reacts") == -1 and youtube.description.lower().find("reaction") == -1 and youtube.title.lower().find("bollywood news mirchi") == -1 and youtube.description.lower().find("bollywood news mirchi") == -1:
	...
	if isfile(filePath) and fileName.endswith(".mp4"):
		...
		if int(clip_size) < MAX_CLIP_LENGTH:
			...
			try:
				...
				os.system("Could not remove trailer")
...
TOKEN_NAME = "the_world_of_dogs_token.json"	# Don't change
...
def uploadYtvid(VIDEO_FILE_NAME='',
		...
		googleAPI.thumbnails().set(
				...
				media_body=MediaFileUpload(current_project_path + "/the_world_of_dogs_thumbnails/thumbnail_" + str(random.randrange(1, 15)) + ".jpg")
		).execute()
