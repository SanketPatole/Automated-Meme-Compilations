...
videos_path = current_project_path + '/videos_funniest_wedding_fail_compilations/'
html = urllib.request.urlopen("https://www.youtube.com/results?search_query=funniest+wedding+fail&sp=EgIIAg%253D%253D")
video_ids = re.findall(r"watch\?v=(\S{11})", html.read().decode())

INTRO_VID = 'funniest_wedding_fail_compilations_intro.mp4'	# SET AS '' IF YOU DON'T HAVE ONE
OUTRO_VID = 'funniest_wedding_fail_compilations_outro.mp4'
TOTAL_VID_LENGTH = 60*60
MAX_CLIP_LENGTH = 10*60
MIN_CLIP_LENGTH = 0
now_timestamp = datetime.datetime.now()
outputFile = "funniest_wedding_fail_compilations_" + num_to_month[now_timestamp.month].upper() + "_" + str(now_timestamp.year) + "_v" + str(
	now_timestamp.day) + ".mp4"

videonumber = date.today() - date(2021, 11, 8)
videonumber = videonumber.days
title = "MEME MARATHON | TRY NOT TO LAUGH | Best Funniest Wedding Fails Compilation Volume #" + str(videonumber)
description = "MEME MARATHON | TRY NOT TO LAUGH | Best Funniest Wedding Fails Compilation Volume #" + str(
	videonumber) + "\n\nEnjoy the memes! :)\n\nlike and subscribe to @Funniest Wedding Fail Compilations for more Videos\n\nCopyright Disclaimer, Under Section 107 of the Copyright Act 1976, allowance is made for 'fair use' for purposes such as criticism, comment, news reporting, teaching, scholarship, and research. Fair use is a use permitted by copyright statute that might otherwise be infringing. Non-profit, educational or personal use tips the balance in favor of fair use.\n\nI as an individual creator do not earn any money from these compilation videos due to 6-7 copyright claims on all of the videos, and YouTube also does not monetize these compilation channels.\nIf you want me to keep making these amazing compilation videos, please become a Patron, support me and do not forget to subscribe to my channel.\nhttps://www.patreon.com/join/funniestmemecompilations/\n\n\n\n#memes #dankmemes #compilation #funny #funnyvideos #wedding #weddingfails\n\n\n\nCredits (Please hit a like to the original videos and subscribe to their channels)\n\n"
tags = ['#memes', '#dankmemes', '#compilation', '#funny', '#funnyvideos', 'memes', 'dankmemes', 'compilation', 'funny',
		'funnyvideos', 'try not to laugh', 'dank memes', 'wedding fails']
...
for video_id in video_ids:
	try:
		...
		if youtube.description.lower().find("react") == -1and  youtube.title.lower().find("react") == -1 and youtube.title.lower().find("react") == -1 and youtube.title.lower().find("bollywood news mirchi") == -1 and youtube.description.lower().find("bollywood news mirchi") == -1:
...
for fileName in videos_list:
	...
	if isfile(filePath) and fileName.endswith(".mp4"):
		...
		if int(clip_size) < MAX_CLIP_LENGTH:
			...
			try:
				os.system("ffmpeg  -i " + filePath + " -t $(( $(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 " + filePath + " |cut -d\. -f1) - 10 )) -c copy temp.mp4 -y;mv temp.mp4 " + filePath)
			except:
...
TOKEN_NAME = "funniest_wedding_fail_compilations_token.json"	# Don't change
...
def uploadYtvid(VIDEO_FILE_NAME='',
		...
		googleAPI.thumbnails().set(
				...
				media_body=MediaFileUpload(current_project_path + "/funniest_wedding_fail_compilations_thumbnails/thumbnail_" + str(random.randrange(1, 14)) + ".jpg")
		).execute()
